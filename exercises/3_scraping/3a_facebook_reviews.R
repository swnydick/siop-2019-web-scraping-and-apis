...
# but possible 504 errors -possible work-arounds

scrape_url <- function(url){
  
  method1 <- function(url){
    download.file(url, destfile = "scrape.html", quiet=TRUE)
    read_html("scrape.html",
              options = c("RECOVER",
                          "HUGE",
                          "NODICT"),
              verbose = TRUE)
  }
  method2 <- function(url){
    message("Hold on a sec -- falling back on curl...")
    Sys.sleep(1)
    # Identify the request as coming from a browser....
    read_html(curl::curl(url, handle = curl::new_handle("useragent" = "Chrome/70.0")))
  }
...
# 6) Save it
saveRDS(all_reviews, "all_fb_reviews.rds")
