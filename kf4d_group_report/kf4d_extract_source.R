####################################
# SOURCE FILE FOR KF4D EXTRACT APP #
#                                  #
# Steven W. Nydick                 #
# 2019-01-18                       #
####################################

# USER: TO ACCESS THE HELP PAGE, UNCOMMENT AND RUN THE FOLLOWING LINE
# vignette("readme_extract", package = "app.kf4d.group.report")

# set working directory to where this source file is
# - if rstudio is running, use that to determine path
# - otherwise, assume you are sourcing the file
src_dir <- tryCatch(dirname(rstudioapi::getSourceEditorContext()$path),
                    error = function(x){
                      getSrcDirectory(function(x) {x})
                    })

# - if there is no path, you need to set the path manually
if(src_dir == ""){
  src_dir <- "~/Documents/kf4d_group_report/"
} # END if STATEMENT

# run the application
kfinstall::launch_application(app_pkg         = "app.kf4d.group.report",
                              app_dir         = src_dir,
                              force_install   = FALSE,
                              launch_function = "launch_extract_application")
